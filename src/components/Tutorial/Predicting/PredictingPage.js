...
export default function PredictingPage({ datasets }) {
  ...
  React.useEffect(() => {
    if (isTraining && newModel) {
      console.log('here new model: ', newModel);
    }
  }, [isTraining, newModel]);
        ...
        {type === "object_segmentation" && (
            <FormModalContextProvider>
              ...
              >
                ...
                <div className={`${step === 2 ? "flex flex-col gap-2" : "hidden"}`}>
                <Input label="Epoch" placeholder="Epoch" name="epoch" required />
                <Input label="Learning Rate" placeholder="Learning Rate" name="learningRate" required />
                </div>
              </ModelingModal>
            </FormModalContextProvider>
          )}
